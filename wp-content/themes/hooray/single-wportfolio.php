            ...
            <?php if(have_posts()): the_post(); ?>
                <article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
                    ...
                    <div class="folio-content">
                        <h3><?php _e( 'Descripción del Proyecto ', 'bd' ) ?></h3>
                        <div class="folio-entry"><?php the_content(); ?></div>

                        <div class="folio-info">
                            <h3><?php _e( 'Informacion', 'bd' ) ?></h3>
                            <ul>
                                <?php
                                if( $address ) {
                                    echo '<li><strong>'. __( 'Direccion', 'bd' ) .'</strong> : <span> '. stripslashes( $address ) .' </span> </li>' ."\n";
                                }

                                if( $phone ) {
                                    echo '<li><strong>'. __( 'Telefono', 'bd' ) .'</strong> : <span> '. stripslashes( $phone ) .' </span> </li>' ."\n";
                                }

                                if( $hos ) {
                                    echo '<li><strong>'. __( 'Horas de Servicio', 'bd' ) .'</strong> : <span> '. stripslashes( $hos ) .' </span> </li>' ."\n";
                                }
                                ...
                                if( $site ) {
                                    echo '<li><strong>'. __( 'Sitio Web', 'bd' ) .'</strong> : <span> '. stripslashes( $site ) .' </span> </li>' ."\n";
                                }

                                if( get_the_term_list($post->ID, 'portfolio_skills', '', '', '') && bdayh_get_option( 'folio_skills' ) ) {
                                    echo '<li class="folio-cat"><strong>'. __( 'Habilidades', 'bd' ) .'</strong> : <span> '. get_the_term_list($post->ID, 'portfolio_skills', '', ', ', '') .' </span> </li>' ."\n";
                                }

                                if( get_the_term_list($post->ID, 'portfolio_category', '', '', '') && bdayh_get_option( 'folio_categories' ) ) {
                                    echo '<li class="folio-cat"><strong>'. __( 'Categorias', 'bd' ) .'</strong> : <span> '. get_the_term_list($post->ID, 'portfolio_category', '', ', ', '') .' </span> </li>' ."\n";
                                }
                                ...
                                if( bdayh_get_option( 'wportfolio_author' ) ) {
                                    echo '<li><strong>'. __( 'Por', 'bd' ) .'</strong> : <span> <a href="'. get_author_posts_url( get_the_author_meta( 'ID' ) ) .'">'. get_the_author_meta( 'display_name' ) .'</a> </span> </li>' ."\n";
                                }
...
//                                    echo '<li><a class="btn-link" href="'. $project_url .'" target="_blank">'. $live_url .'</a> </li>' ."\n";
                ...
                <?php if( $projects->have_posts() && bdayh_get_option( 'folio_related' ) ){ ?>
                    <div class="folio-related">
                        <div class="box-title"> <h2><?php _e( 'Articulos Relacionados', 'bd' ) ?></h2> </div>
                    </div><!-- .folio-related -->
